...
public partial class IntegrationTests
{
    ...
    {
        var type = _testResult.Assembly.GetType("StructWithArray");
        dynamic first = Activator.CreateInstance(type);
        first.X = new[] { 1, 2 };
        first.Y = new[] { 3, 4 };
        dynamic second = Activator.CreateInstance(type);
    ...
    {
        var leftType = _testResult.Assembly.GetType(left);
        dynamic leftInstance = Activator.CreateInstance(leftType);
        leftInstance.A = 1;

        var rightType = _testResult.Assembly.GetType(right);
    ...
    {
        var type = _testResult.Assembly.GetType("StructPropertyClass");
        var propertyType = _testResult.Assembly.GetType("SimpleStruct");
        dynamic first = Activator.CreateInstance(type);
        first.A = 1;
